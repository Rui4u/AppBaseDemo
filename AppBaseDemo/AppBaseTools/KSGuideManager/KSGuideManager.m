...
- (void)myInit {
    ...
    self.lookBtn.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.frame = CGRectMake(0, SCREEN_HEIGHT - AutoHeight(80), kScreenBounds.size.width, 44.0f);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == self.images.count - 1) {
        ...
        [cell.lookBtn setHidden:NO];

        [cell.lookBtn addTarget:self action:@selector(nextButtonHandler:) forControlEvents:UIControlEventTouchUpInside];
    } else {

        [cell.lookBtn setHidden:YES];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    DDLog(@"%f",scrollView.contentOffset.x / kScreenBounds.size.width);
    if(scrollView.contentOffset.x / kScreenBounds.size.width == 3)
    {
